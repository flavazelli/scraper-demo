# Description: This file contains the scraper for the Provigo website. The scraper will scrape the product name, brand, size, regular price, sale price, price per kg, price per unit, price per 100g, and price per 100ml. The scraper will then store the data in the database.
# The scraper is launched by the launch_scraper function in scraper_utils.py
# The scraper iterates through the pages of the Provigo website and extracts the product information from the HTML using BeautifulSoup.
# The product information is then stored in a MongoDB database.
import re
from scraper_utils import launch_scraper

def scrape_provigo(product, item):
...
launch_scraper('https://www.provigo.ca/en/food/c/27985?page=', 'chakra-linkbox', 'provigo', scrape_provigo)
